...
class SprModel_frame:
	...
	def initUI(self):
		...
		self.compute.grid(row=10, column=0, padx=5, pady=(10,2), sticky=tk.W)

		#BUTTON TO PLAY SINE OUTPUT
		output_label = "Sinusoidal:"
		tk.Label(self.parent, text=output_label).grid(row=11, column=0, sticky=tk.W, padx=5, pady=(10,0))
		self.output = tk.Button(self.parent, text=">", command=lambda:UF.wavplay('output_sounds/' + os.path.basename(self.filelocation.get())[:-4] + '_sprModel_sines.wav'), bg="gray30", fg="white")
		self.output.grid(row=11, column=0, padx=(80,5), pady=(10,0), sticky=tk.W)

		#BUTTON TO PLAY RESIDUAL OUTPUT
		output_label = "Residual:"
		tk.Label(self.parent, text=output_label).grid(row=12, column=0, sticky=tk.W, padx=5, pady=(5,0))
		self.output = tk.Button(self.parent, text=">", command=lambda:UF.wavplay('output_sounds/' + os.path.basename(self.filelocation.get())[:-4] + '_sprModel_residual.wav'), bg="gray30", fg="white")
		self.output.grid(row=12, column=0, padx=(80,5), pady=(5,0), sticky=tk.W)

		#BUTTON TO PLAY OUTPUT
		output_label = "Output:"
		tk.Label(self.parent, text=output_label).grid(row=13, column=0, sticky=tk.W, padx=5, pady=(5,15))
		self.output = tk.Button(self.parent, text=">", command=lambda:		UF.wavplay('output_sounds/' + os.path.basename(self.filelocation.get())[:-4] + '_sprModel.wav'), bg="gray30", fg="white")
		self.output.grid(row=13, column=0, padx=(80,5), pady=(5,15), sticky=tk.W)
