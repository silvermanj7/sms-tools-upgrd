...
class HpsTransformations_frame:
	...
	def initUI(self):
		...
		#DECIMATION FACTOR
		stocf_label = "stocf:"
		tk.Label(self.parent, text=stocf_label).grid(row=3, column=0, sticky=tk.W, padx=(250,5), pady=(10,2))
		self.stocf = tk.Entry(self.parent, justify=tk.CENTER)
		self.stocf["width"] = 5
		self.stocf.grid(row=3, column=0, sticky=tk.W, padx=(290,5), pady=(10,2))
		...
		self.transf_output = tk.Button(self.parent, text=">", command=lambda:UF.wavplay('output_sounds/' + os.path.basename(self.filelocation.get())[:-4] + '_hpsModelTransformation.wav'), bg="gray30", fg="white")
